...
def gdrive_fix(url):
    ...
    try:
        ...
        if file_id:
            # Format baru: pake 'lh3.googleusercontent.com' biasanya lebih ampuh
...
# Dashboard Saldo (Admin di semua menu kecuali Inv/Pus, User di menu Laporan)
...
if menu == "📚 Pustaka":
    ...
    # Fitur Tambah Materi (Hanya Admin)
    if st.session_state['role'] == "admin":
...
# --- MENU LAINNYA (TETAP SAMA SEPERTI ORDER SEBELUMNYA) ---
elif menu == "📊 Laporan":
    ...
    with t2:
        if not df_event.empty:
            ...
            st.write("**📥 Iuran:**"); st.dataframe(df_event[df_event['Nama Event'] == ev_sel][['Tanggal', 'Nama', 'Jumlah']], hide_index=True)
    with t3:
        ck, ch = st.columns(2)
    ...
    with st.form("f_out", clear_on_submit=True):
        ev_ref = "N/A"
        if kat_pilih == "Event": ev_ref = st.selectbox("Wajib Pilih Event:", ["-- Pilih Event --"] + (df_event['Nama Event'].unique().tolist() if not df_event.empty else []))
    ...
    with tab_view:
        if not df_inv.empty: st.dataframe(df_inv, hide_index=True, use_container_width=True)
    with tab_add:

...
if menu == "📚 Pustaka":
    ...
    if st.session_state['role'] == "admin":
        with st.expander("➕ Tambah Materi Baru"):
            if 'Kegiatan' in df_pus.columns and not df_pus.empty:
                kegiatan_lama = df_pus['Kegiatan'].dropna().astype(str).str.strip()
                list_kegiatan = sorted([x for x in kegiatan_lama.unique().tolist() if x])
            else:
                list_kegiatan = []

            kegiatan_options = ["-- Tidak ada kegiatan --"] + list_kegiatan + ["➕ Tambah Kegiatan Baru"]
            pilih_kegiatan_baru = st.selectbox("Kegiatan", kegiatan_options, key="pilih_kegiatan_baru")

            kegiatan_baru = ""
            if pilih_kegiatan_baru == "➕ Tambah Kegiatan Baru":
            ...
            with st.form("f_add_pus", clear_on_submit=True):
                ...
                k_p = st.selectbox("Kategori", ["Kitab", "Rekaman Audio", "Video", "Foto Kegiatan", "Dokumen"])
                l_p = st.text_input("Link G-Drive/URL")
                t_p = st.selectbox("Tipe File", ["PDF", "Gambar", "Foto", "Video", "Audio", "Link"])
            ...
            if simpan:
                ...
                elif pilih_kegiatan_baru == "-- Tidak ada kegiatan --":
                    kegiatan_final = ""
                else:
                    kegiatan_final = pilih_kegiatan_baru.strip()
                ...
                if not j_p.strip():
                    ...
                    st.error("Foto/Video wajib memiliki Kegiatan. Silakan pilih kegiatan atau buat kegiatan baru.")
                else:
                    # A=Judul, B=Kategori, C=Link, D=Tipe, E=Deskripsi, F=Kegiatan
    ...
    if not df_pus.empty:
        ...
        if cari:
            cari_l = cari.lower()
            mask = df_view.apply(
                lambda row: cari_l in str(row['Judul']).lower() or cari_l in str(row['Deskripsi']).lower(),
                axis=1
            )
        ...
        if not galeri_df.empty:
            ...
            if 'Kegiatan' in galeri_df.columns:
                kegiatan_series = galeri_df['Kegiatan'].fillna("").astype(str).str.strip()
            else:
                kegiatan_series = pd.Series([""] * len(galeri_df), index=galeri_df.index)
                    ...
                    with cols[i % 3]:
                        ...
                        if tipe_g == "Foto":
                            ...
                            if file_id:
                                ...
                                st.markdown(
                                    f'<iframe src="{preview_url}" width="100%" height="300" allow="autoplay" allowfullscreen style="border:none;border-radius:10px;"></iframe>',
                                    unsafe_allow_html=True
                                )
                                st.link_button("▶️ Buka Video di Google Drive", url_g)
                            else:
        ...
        if not pustaka_df.empty:
            ...
            for _, row in pustaka_df.iterrows():
                with st.container():
                    ...
                    tipe = str(row['Tipe'])
                    icon = "📄" if tipe == "PDF" else "🖼️" if tipe == "Gambar" else "🔊" if tipe == "Audio" else "🔗"
                    col1.markdown(f"<h1 style='text-align: center;'>{icon}</h1>", unsafe_allow_html=True)
                    col2.write(f"### {row['Judul']}")
                    col2.caption(f"Kategori: {row['Kategori']} | Tipe: {tipe}")
                    col2.write(str(row['Deskripsi']))
                    with col2.expander("Lihat / Putar Materi"):
                        if tipe == "Audio":
                            p_url = str(row['Link']).replace('/view', '/preview')
                            st.markdown(
                                f'<iframe src="{p_url}" width="100%" height="150" style="border:none;border-radius:10px;"></iframe>',
                                unsafe_allow_html=True
                            )
                            st.link_button("🚀 Putar di G-Drive", row['Link'])
                        elif tipe == "PDF":
                            st.markdown(
                                f'<iframe src="{gdrive_fix(row["Link"])}" width="100%" height="500px"></iframe>',
                                unsafe_allow_html=True
                            )
                        elif tipe == "Gambar":
                            url_g = str(row['Link'])
                            ...
                            st.link_button("🔗 Buka Materi", row['Link'])
                st.divider()

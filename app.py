...
# --- 🔥 FUNGSI LOGGING (OPERASIONAL) ---
def log_aksi(sheet_log, data_list):
    try: sh.worksheet(sheet_log).append_row([datetime.now().strftime("%d/%m/%Y %H:%M"), st.session_state['role']] + data_list)
    except: pass
...
with st.sidebar:
    ...
    if st.session_state['logged_in']:
        ...
        if st.button("Log Out ➔", key="btn_out"):
            st.session_state.update({'logged_in': False, 'role': 'user'}); st.rerun()
    else:
...
if (st.session_state['role'] == "admin" and menu not in ["📦 Inventaris", "📚 Pustaka"]) or (st.session_state['role'] == "user" and menu == "📊 Laporan"):
...
if menu == "📚 Pustaka":
    ...
    if not df_pus.empty:
        ...
        cari, sel_k = c_search.text_input("🔍 Cari Materi"), c_filter.selectbox("📂 Filter Kategori", ["Semua"] + df_pus['Kategori'].unique().tolist())
        df_view = df_pus.copy()
        if sel_k != "Semua": df_view = df_view[df_view['Kategori'] == sel_k]
        if cari: df_view = df_view[df_view.apply(lambda r: cari.lower() in r['Judul'].lower() or cari.lower() in r['Deskripsi'].lower(), axis=1)]
        for _, row in df_view.iterrows():
            with st.container():
                ...
                with col2.expander("Lihat / Putar Materi"):
                    if row['Tipe'] == "Audio": st.markdown(f'<iframe src="{row["Link"].replace("/view", "/preview")}" width="100%" height="150" style="border:none; border-radius:10px;"></iframe>', unsafe_allow_html=True); st.link_button("🚀 Putar di G-Drive", row['Link'])
                    elif row['Tipe'] == "PDF": st.markdown(f'<iframe src="{gdrive_fix(row["Link"])}" width="100%" height="500px"></iframe>', unsafe_allow_html=True)
                    elif row['Tipe'] == "Gambar": st.image(gdrive_fix(row['Link']), use_container_width=True); st.link_button("📂 Buka Gambar Asli", row['Link'])
            st.divider()
    ...
    with t1:
        ...
        if not df_y.empty and not df_warga.empty:
            ...
            st.write("#### 🟢 Kas (Target: 15rb)"); st.dataframe(warna_iuran(df_y.pivot_table(index='Nama', columns='Bulan', values='Kas', aggfunc='sum', observed=False).fillna(0).astype(int), 15000), use_container_width=True)
            st.write("#### 🟡 Hadiah (Target: 35rb)"); st.dataframe(warna_iuran(df_y.pivot_table(index='Nama', columns='Bulan', values='Hadiah', aggfunc='sum', observed=False).fillna(0).astype(int), 35000), use_container_width=True)
    with t2:
        if not df_event.empty:
            ...
            c1.metric("Total Iuran", f"Rp {int(df_ev_masuk['Jumlah'].sum()):,}"); c2.metric("Total Belanja", f"Rp {int(df_ev_keluar['Jumlah'].sum()):,}"); c3.metric("Sisa Saldo", f"Rp {int(df_ev_masuk['Jumlah'].sum() - df_ev_keluar['Jumlah'].sum()):,}")
            st.divider(); col_in, col_out = st.columns(2); col_in.write("#### 📥 Pemasukan"); col_in.dataframe(df_ev_masuk[['Tanggal', 'Nama', 'Jumlah']], hide_index=True); col_out.write("#### 📤 Pengeluaran"); col_out.dataframe(df_ev_keluar[['Tanggal', 'Keterangan', 'Jumlah']], hide_index=True)
    with t3:
        ck, ch = st.columns(2); ck.write("#### 📤 Pengeluaran KAS"); ck.dataframe(df_keluar[df_keluar['Kategori'] == 'Kas'][['Tanggal', 'Jumlah', 'Keterangan']], hide_index=True); ch.write("#### 📤 Pengeluaran HADIAH"); ch.dataframe(df_keluar[df_keluar['Kategori'] == 'Hadiah'][['Tanggal', 'Jumlah', 'Keterangan']], hide_index=True)
    ...
    with tab_edit:
        ...
        if not df_inv.empty:
            ...
            with st.expander("📝 Update Status / Peminjaman", expanded=True):
                with st.form("f_inv_edit"):
                    ...
                    n_lok, n_ket = st.text_input("Update Nama Lokasi", value=curr['Lokasi']), st.text_input("Peminjam / Keperluan", value=curr['Keterangan'])
            ...
            with st.expander("📦 Pindah Sebagian ke Lokasi Lain"):
                ...
                if int(curr_p['Jumlah']) <= 1: st.info("💡 Stok cuma 1 unit. Gunakan 'Update Status' untuk ganti lokasi.")
                else:
                    with st.form("f_pindah_lokasi"):
                        c_jml, c_lok = st.columns(2)
                        jml_pindah, lok_baru = c_jml.number_input("Jumlah", 1, int(curr_p['Jumlah'])-1), c_lok.text_input("Lokasi Tujuan")
            ...
            with st.expander("🛠️ Pecah Kondisi (Misal: Sebagian Rusak)"):
                ...
                if int(curr_k['Jumlah']) <= 1: st.info("💡 Stok cuma 1 unit. Gunakan 'Update Status' untuk ganti kondisi.")
                else:
                    with st.form("f_pecah_kondisi"):
                        c_jml_r, c_kon_r = st.columns(2)
                        jml_berubah, kon_baru = c_jml_r.number_input("Jumlah Rusak", 1, int(curr_k['Jumlah'])-1), c_kon_r.selectbox("Kondisi", ["Rusak Ringan", "Rusak Parah"])
        ...
        if sukses_update or sukses_pindah or sukses_kondisi: st.success("✅ Berhasil!"); st.cache_data.clear(); time.sleep(1); st.rerun()

elif menu == "📥 Kas Bulanan" and st.session_state['role'] == "admin":
    st.subheader("📥 Input Pembayaran Iuran")
    w_options = sorted([f"{r['Nama']} ({r['Role']})" for _, r in df_warga.iterrows()])
    sel_w = st.selectbox("Pilih Warga", w_options).split(" (")[0]
    mode = st.radio("Mode:", ["Paket Lengkap (50rb)", "Hanya Kas (15rb)", "Hanya Hadiah (35rb)", "Custom Nominal"], horizontal=True)
    n_val = 50000 if "Paket" in mode else 15000 if "Kas" in mode else 35000 if "Hadiah" in mode else 0
    with st.form("f_kas", clear_on_submit=True):
        c1, c2, c3 = st.columns([2, 1, 1])
        n, t_in, b_in = c1.number_input("Nominal", value=n_val), c2.selectbox("Tahun", range(2022, 2031), index=4), c3.selectbox("Bulan", bln_list)
        if st.form_submit_button("🚀 Proses"):
            uang_sisa, b_idx, t_jalan = n, bln_list.index(b_in), t_in
            role_w = df_warga[df_warga['Nama'] == sel_w]['Role'].values[0] if not df_warga[df_warga['Nama'] == sel_w].empty else "Main Warga"
            while uang_sisa > 0:
                c_bln = bln_list[b_idx]
                df_curr = df_masuk[(df_masuk['Nama'] == sel_w) & (df_masuk['Tahun'] == t_jalan) & (df_masuk['Bulan'] == c_bln)]
                k_ter, h_ter = df_curr['Kas'].sum(), df_curr['Hadiah'].sum()
                j_k, j_h = (max(0, 15000 - k_ter) if "Hadiah" not in mode else 0), (max(0, 35000 - h_ter) if "Kas" not in mode else 0)
                if j_k <= 0 and j_h <= 0:
                    b_idx += 1; 
                    if b_idx >= 12: b_idx = 0; t_jalan += 1
                    continue
                p_k, p_h = min(uang_sisa, j_k), min(max(0, uang_sisa - min(uang_sisa, j_k)), j_h)
                uang_sisa -= (p_k + p_h)
                if (p_k + p_h) > 0:
                    sh.worksheet("Pemasukan").append_row([datetime.now().strftime("%d/%m/%Y"), sel_w, t_jalan, c_bln, int(p_k+p_h), int(p_k), int(p_h), "LUNAS" if role_w != "Warga Support" else "PARTISIPASI", mode.split(" (")[0]])
                b_idx += 1
                if b_idx >= 12: b_idx = 0; t_jalan += 1
    ...
    with st.form("f_out", clear_on_submit=True):
        ev = st.selectbox("Event:", ["N/A"] + (df_event['Nama Event'].unique().tolist() if not df_event.empty else [])) if kat == "Event" else "N/A"
        nom, ket = st.number_input("Nominal"), st.text_input("Keterangan")
        if st.form_submit_button("Simpan"):
            sh.worksheet("Pengeluaran").append_row([datetime.now().strftime("%d/%m/%Y"), kat, int(nom), f"[{ev}] {ket}" if kat == "Event" else ket])
    ...
    st.subheader("📜 Log Aktivitas Terakhir")
    try:
        st.write("#### 📥 Pemasukan Terbaru")
        st.dataframe(df_masuk.iloc[::-1].head(20), hide_index=True, use_container_width=True)
        st.write("#### 📤 Pengeluaran Terbaru")
        st.dataframe(df_keluar.iloc[::-1].head(10), hide_index=True, use_container_width=True)
